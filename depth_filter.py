...
class DepthFilter:
    def __init__(
        ...
        color_image_dir,
        depth_image_dir,
        cube_screen_image_dir,
        cube_depth_image_dir,
        translation,
        axis_vec,
    ):
    ...
    def generate_pcd(self):
        ...
        pinhole_camera_intrinsic = o3d.camera.PinholeCameraIntrinsic(
            width=640,
            height=480,
            fx=380.2484436035156,
            fy=379.8265380859375,
            cx=314.2138977050781,
            cy=240.59800720214844,
        )

        pcd = o3d.geometry.PointCloud.create_from_rgbd_image(
            rgbd, intrinsic=pinhole_camera_intrinsic, extrinsic=self.extrinsic
        )
        return pcd
    ...
    def crop(self):
        ...
        (
            ...
            left_top,
            right_top,
        ) = get_bounding_box_world_coordinates()
        ...
        left_bottom_pcl_ = np.array(
            (
                left_bottom[0],
                left_bottom[1],
                left_bottom[2] - BOX_LENGTH - HEIGHT_BUFFER,
            )
        )
        right_bottom_pcl_ = np.array(
            (
                right_bottom[0],
                right_bottom[1],
                right_bottom[2] - BOX_LENGTH - HEIGHT_BUFFER,
            )
        )
        left_top_pcl_ = np.array(
            (left_top[0], left_top[1], left_top[2] - BOX_LENGTH - HEIGHT_BUFFER)
        )
        right_top_pcl_ = np.array(
            (right_top[0], right_top[1], right_top[2] - BOX_LENGTH - HEIGHT_BUFFER)
        )
        corners = np.array(
            [
                left_bottom_pcl,
        ...
        coord_obb = (
            mesh_frame.get_axis_aligned_bounding_box().get_oriented_bounding_box()
        )
        # Rotate oriented_bounding_box to mesh_frame
        bounding_points = o3d.utility.Vector3dVector(bounding_polygon)
        oriented_bounding_box = o3d.geometry.OrientedBoundingBox.create_from_points(
            bounding_points
        )
        relative_rotation = coord_obb.R @ np.linalg.inv(oriented_bounding_box.R)
        oriented_bounding_box.rotate(
            R=relative_rotation, center=oriented_bounding_box.center
        )
        ...
        (
            ...
            plank_left_top,
            plank_right_top,
        ) = get_plank_bounding_box_world_coordinates()
        plank_left_bottom_pcl = np.array(
            (plank_left_bottom[0], plank_left_bottom[1], plank_left_bottom[2])
        )
        plank_right_bottom_pcl = np.array(
            (plank_right_bottom[0], plank_right_bottom[1], plank_right_bottom[2])
        )
        plank_left_top_pcl = np.array(
            (plank_left_top[0], plank_left_top[1], plank_left_top[2])
        )
        plank_right_top_pcl = np.array(
            (plank_right_top[0], plank_right_top[1], plank_right_top[2])
        )
        plank_left_bottom_pcl_ = np.array(
            (
                plank_left_bottom[0],
                plank_left_bottom[1],
                plank_left_bottom[2] - BOX_LENGTH - HEIGHT_BUFFER,
            )
        )
        plank_right_bottom_pcl_ = np.array(
            (
                plank_right_bottom[0],
                plank_right_bottom[1],
                plank_right_bottom[2] - BOX_LENGTH - HEIGHT_BUFFER,
            )
        )
        plank_left_top_pcl_ = np.array(
            (
                plank_left_top[0],
                plank_left_top[1],
                plank_left_top[2] - BOX_LENGTH - HEIGHT_BUFFER,
            )
        )
        plank_right_top_pcl_ = np.array(
            (
                plank_right_top[0],
                plank_right_top[1],
                plank_right_top[2] - BOX_LENGTH - HEIGHT_BUFFER,
            )
        )
        corners_ = np.array(
            [
                plank_left_bottom_pcl,
                plank_right_bottom_pcl,
                plank_left_top_pcl,
                plank_right_top_pcl,
                plank_left_bottom_pcl_,
                plank_right_bottom_pcl_,
                plank_left_top_pcl_,
                plank_right_top_pcl_,
            ]
        )
        plank_bounding_polygon = corners_.astype("float64")
        plank_bounding_points = o3d.utility.Vector3dVector(plank_bounding_polygon)
        plank_oriented_bounding_box = (
            o3d.geometry.OrientedBoundingBox.create_from_points(plank_bounding_points)
        )
        relative_rotation_ = coord_obb.R @ np.linalg.inv(plank_oriented_bounding_box.R)
        plank_oriented_bounding_box.rotate(
            R=relative_rotation_, center=plank_oriented_bounding_box.center
        )
        ...
        if (
            np.asarray(cropped_pcd_.points).shape[0]
            > np.asarray(cropped_pcd.points).shape[0]
        ):
...
def get_bounding_box_world_coordinates():
    ...
    left_bottom = [
        CAMERA_X + 0.41,
        -TABLE_WIDTH / 2 - WIDTH_BUFFER,
        -ROBOT_TO_HEIGHT + BOX_LENGTH + HEIGHT_BUFFER,
    ]
    right_bottom = [
        CAMERA_X + 0.41,
        TABLE_WIDTH / 2 + WIDTH_BUFFER,
...
def get_plank_bounding_box_world_coordinates():
    ...
    left_bottom = [
        CAMERA_X + 0.41,
        -TABLE_WIDTH / 2 - buffer,
        -ROBOT_TO_HEIGHT + BOX_LENGTH + HEIGHT_BUFFER_,
    ]
    right_bottom = [
        CAMERA_X + 0.41,
        TABLE_WIDTH / 2 + buffer,
