...
def bag_to_depth_images(rosbag_name, depth_ros_topic, depth_dir, start_time, end_time, img_dir, bundletrack_depth_dir):
...
def extract_cube_pose(start_time, end_time, bagfile, odom_bagfile, odom_topic):
    """
    ...
    for topic, msg, t in odom_bag.read_messages(topics=str(odom_topic)):
        if msg.header.stamp.secs in final_odom_timestamps.keys() and msg.header.stamp.nsecs in final_odom_timestamps[msg.header.stamp.secs]:
            ...
            print("Saving the timestamp at: ", msg.header.stamp.secs, msg.header.stamp.nsecs)
            break#only need pose for the first frame
    depth_bag.close()
    odom_bag.close()
    return

def extract_gt_poses_from_tagslam(start_time, end_time, depth_bag_file, odom_bag_file, depth_topic, odom_topic, output_dir):
    """
    ...
    for (topic, msg, ts) in depth_bag.read_messages(topics=str(depth_topic)):
        ...
        if msg.header.stamp.secs not in depth_timestamps.keys():
            depth_timestamps[msg.header.stamp.secs] = []
        depth_timestamps[msg.header.stamp.secs].append(msg.header.stamp.nsecs)
    print("The length of the depth_timestamps is {}".format(getDictLen(depth_timestamps)))
    ...
    for secs in tqdm(depth_timestamps.keys()):
        ...
        for nsecs in depth_timestamps[secs]:
            ...
            if result_nsecs in final_odom_timestamps[secs]:
                continue
            final_odom_timestamps[secs].append(result_nsecs)
    print("The length of the final_odom_timestamps is {}".format(getDictLen(final_odom_timestamps)))
