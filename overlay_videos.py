...
class OverlayVideoGenerator:
    ...
    def __init__(self, vision_asset: str, cycle_iteration: int,
        ...
        if cycle_iteration == 0:
            ...
            print('The current implementation of tagslam pose visualization' + \
                  ' may be incorrect. ')
    ...
    def _get_bundletrack_poses_in_cam(self) -> None:
        if self.tracking_bundlesdf_id is None:
            print(f'WARNING:  no BundleSDF tracking results to visualize.')
            return
    ...
    def _add_meshcat_objects(self, vis: meshcat.Visualizer) -> None:
        ...
        vis["bundlesdf_mesh"].set_object(
            ...
            g.MeshLambertMaterial(
                color=self.mesh_color, reflectivity=0.0, transparent=0,
    ...
    def _configure_lighting(self, vis: meshcat.Visualizer) -> None:
        vis["/Lights/FillLight/<object>"].set_property("intensity", 1)
        vis["/Lights/FillLight/<object>"].set_transform(
            self.T_MC @ tf.translation_matrix([1, 1, 0.5]))
    ...
    def make_overlay_video(self, to_pll_input_dir=False) -> None:
        ...
        with TemporaryDirectory(prefix="ros-process-") as tmpdir:
            ...
            if not to_pll_input_dir:
                output_file = self.output_file
            else:
                # Put the output video in the same directory as the pll inputs
                # though pll does not need the videos. It is for easier
                # visualization.
                output_file = \
                    file_utils.contactnets_input_bsdf_overlay_video_path(
                        self.vision_asset, self.cycle_iteration,
                        self.tracking_bundlesdf_id, self.nerf_bundlesdf_id,
                        gt_mesh=self.gt_mesh, with_robot=self.show_robot)
