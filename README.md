...
Right now all the procedures below are implemented in `example.py`. You may replace the paths including `ROOT_DIR` and `BUNDLETRACK_DATA_DIR` with local directory for dataset and BundleTrack data directory, and replace `CAMERA_CONFIG` with correct extrinsic parameters and simply run `example.py` to finish data generation.
