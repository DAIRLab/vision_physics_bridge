...
class TrajectoryConverterBundleSDFToPLL:
    ...
    def __init__(self, tracking_bundlesdf_id: str, nerf_bundlesdf_id: str,
        ...
        if object in file_utils.TAGLESS_OBJECTS and not bsdf_only:
            bsdf_only = True
    ...
    def do_process(self) -> None:
        """Generate contactnets-format trajectories for BundleSDF outputs and
        also for TagSLAM outputs, if not self.bsdf_only.  This requires
        estimating velocities from differences in pose, converting everything to
        PLL format, and trimming the trajectories to view the autonomous
        dynamics during the toss only.  The PLL format is in:
	        [ quaternion  position  angular_velocity  linear_velocity ]
        where:
            - quaternion: 		[qw, qx, qy, qz]
            - position:  		[x, y, z] in meters
