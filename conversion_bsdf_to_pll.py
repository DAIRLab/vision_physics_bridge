...
def build_pll_robot_object_tensordict(
        object_state: np.ndarray, robot_state: np.ndarray) -> TensorDictBase:
    ...
    return TensorDict(
        {'object_state': torch.tensor(object_state),
         'robot_state': torch.tensor(robot_state)}, [n_horizon])
...
class TrajectoryConverterBundleSDFToPLL:
    ...
    def _load_poses(self) -> None:
        ...
        directory's files synced_joint_angles.txt and
        synced_joint_velocities.txt, which share the timestamps of the
    ...
    def _load_robot_joint_states(self) -> None:
        """Load the robot joint angles and velocities from the synchronized
        joint angles and velocities files."""
        ...
        assert robot_joint_velocities.shape == robot_joint_angles.shape, \
            f'{robot_joint_velocities.shape=} != {robot_joint_angles.shape=}'
